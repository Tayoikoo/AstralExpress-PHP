...
class OnPlayerHeartBeat
{
    ...
    public function handle(ConnectionInterface $socket, Message $request): void
    {
        ...
        $uidScript = "
            CS.UnityEngine.GameObject.Find('UIRoot/AboveDialog/BetaHintDialog(Clone)'):GetComponentInChildren(typeof(CS.RPG.Client.LocalizedText)).text = '<size=20>AstralPHP is written in PHP $php_version</size>'
            CS.UnityEngine.GameObject.Find('VersionText'):GetComponentInChildren(typeof(CS.RPG.Client.LocalizedText)).text = '<size=17><color=#FFC0CB>Rewritten version of VirusSR</color></size>'
        ";
