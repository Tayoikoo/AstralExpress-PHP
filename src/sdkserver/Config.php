...
class Config {
    ...
    public static function load_config($file_path) {
        ...
        $config_object->versions = [];
        foreach ($config['versions'] as $versionName => $versionData) {
            $config_object->versions[$versionName] = new VersionConfig($versionData);
        }

        Logger::log_dispatch("Loaded versions: " . json_encode(array_keys($config_object->versions)));
